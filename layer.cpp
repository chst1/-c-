...
CNN_layer::~CNN_layer()
{
    ...
    delete_mem<float>(w, ksize_x, ksize_y, output_channel, input_channel);
    delete_mem<float>(dw, ksize_x, ksize_y, output_channel, input_channel);
    delete_mem<float>(W_r, ksize_x, ksize_y, output_channel, input_channel);
    delete_mem<float>(W_s, ksize_x, ksize_y, output_channel, input_channel);
    delete[] b;
    delete[] db;
    delete[] B_r;
    delete[] B_s;
                        ...
                        else
                        {
                            for (int n = 0; n < input_y; n++)
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        dw[x][y][i][j] += input[k][m_x][n_y][j] * for_input[k][m_x][n_y][i];
                                    }
                                }
                            }
                        }
...
void CNN_layer::optim(float lr, float p1 = 0.9, float p2 = 0.99)
{
    t++;
...
void Fully_layer::backward(float **input, float **for_input)
{
    ...
    for (int i = 0; i < input_channel; i++)
    {
        for (int j = 0; j < output_channel; j++)
        {
            for (int k = 0; k < batch; k++)
            {
                dw[i][j] += for_input[k][j] * input[k][i];
            }
        }
    }
    // get db
...
void Fully_layer::optim(float lr, float p1 = 0.9, float p2 = 0.99)
{
    // update dw_r, dw_s, w;
    t++;
    float dev_s = 1 - quite_m(p1, t);
...
void loss::backward(float **y_pre, int *y_true)
{
    ...
    for(int i=0;i<batch;i++)
    {
        dx[i][y_true[i]] = -1.0/(y_pre[i][y_true[i]]+1e-8);
    }
}
